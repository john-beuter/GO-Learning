...
func init() {
	...
	tmpListenPort := os.Getenv("BOOM_LISTEN_PORT") 											//How are these getter getting set
	if tmpListenPort != "" {
...
func connectionHandler(conn net.Conn) {
	...
	log.Printf("Received message from %v: %v\n", clientIP, base64.StdEncoding.EncodeToString(buf[:sizeRead])) 			//Receives the connection over the wire for the detentation
	detCode := buf[:sizeRead]
	err = detonateBomb(string(detCode))
	if err != nil {
		log.Printf("Error detonating WMD: %v\n", err)
	} else {
		log.Println("Successfully ran detonation safety script") 									//What is the saftey script? 
	}
